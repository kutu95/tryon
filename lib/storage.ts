...
export async function getSignedUrl(bucket: string, path: string, expiresIn: number = 3600): Promise<string | null> {
  try {
    ...
    // Clean the path - remove any leading slashes and ensure it's properly formatted
    ...
    if (error) {
      console.error('[Storage] Error creating signed URL:', {
        bucket,
        path: cleanPath,
        error: error.message,
    ...
    if (!data || !data.signedUrl) {
      console.error('[Storage] No signed URL returned:', { bucket, path: cleanPath, data })
      return null
    }
    ...
    console.error('[Storage] Exception in getSignedUrl:', {
      bucket,
      path,
      error: error.message,
      stack: error.stack
